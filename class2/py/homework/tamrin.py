...
#train test-split//kfold//leavepout
...
print("so the best way is to train with leave one out")
...
for train_index, test_index in selector.split(X):
    ...
    y_predict=model.predict(X_test)
    error=mean_squared_error(y_test,y_predict)
    if error ==np.min(error_list_leaveone):
        print(train_index)
...
""" for i in range(1,50):
    ...
    y_predict=model.predict(x_test)
    error=mean_squared_error(y_test,y_predict)
    if error == np.min(error_list):
        print("behterin training ba test size ",i*2," darsad rokh midahad")
 """